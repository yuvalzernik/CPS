...
public class InputValidator {
	...
	public static boolean OrderInAdvance(String customerId, String carNumber, String email, LocalDate arrivalDate,
			LocalDate leavingDate, String arrivalHour, String leavingHour) {
		
		if (CarNumber(carNumber) && StartingDate(arrivalDate) && Id(customerId) && CheckHourFormat(arrivalHour)
				&& CheckHourFormat(leavingHour) && CheckLeavingHour(arrivalHour, leavingHour, arrivalDate, leavingDate)
				&& Email(email) && CheckLeavingDate(arrivalDate, leavingDate))
					return true;
			return false;
	}
}
